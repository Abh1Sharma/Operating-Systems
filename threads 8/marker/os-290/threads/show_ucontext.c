...
int main(int argc, char **argv)
{
	...
	err = getcontext(&mycontext);   //abhi added -> reference: ucontext_t mycontext = { 0 };
	assert(!err);
...
//	TBD();
	...
	printf("ucontext_t size = %ld bytes\n", (long int)sizeof(mycontext)); // mycontext is a pointer to the struct ucontext
	...
	printf("memory address of the program counter (RIP) saved "
	       "in mycontext = 0x%lx\n",
	       (unsigned long) mycontext.uc_mcontext.gregs[REG_RIP] ); // iffy about how to do this
	...
	printf("number of bytes pushed to the stack between setcontext_called "
	       "and err = %ld\n",   (unsigned long)( abs((unsigned long)&setcontext_called - (unsigned long)&err))   ); // sizeof()
	...
	printf("number of bytes between err and the saved stack in mycontext "
	       "= %ld\n", (unsigned long) abs(   (unsigned long)&err - (unsigned long)mycontext.uc_mcontext.gregs[REG_RSP]   )  );
...
show_interrupt(void)
{
	...
	interrupts_off();// interrupts called off, affects stack.
